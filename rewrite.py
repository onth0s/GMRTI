...
#   - heading_transform: optional tuple of (pattern, replacement)
ASSEMBLY_MANIFEST = (
    {"path": "src/00_preamble.md",           "merge_mode": "separator"},
    {
        "path": "GLOSSARY.md",
        "merge_mode": "append",
        "heading_transform": (r"^#\s+Glossary\b", "**0.4 Vocabulary**"),
    },
...
def _list_monolithic_files(base_dir):
    """Return a sorted list of monolithic revision filenames in base_dir."""
    return sorted(
        [f for f in os.listdir(base_dir) if MONOLITHIC_PATTERN.match(f)]
    )
...
def assemble_monolithic_document(base_dir):
    ...
    for entry in ASSEMBLY_MANIFEST:
        ...
        transform = entry.get("heading_transform")
        if transform:
...
def archive_prior_monolithic_files(base_dir, current_output_path=None):
    ...
    archived_files = []
    for f in _list_monolithic_files(base_dir):
...
def check_monolithic_sync(base_dir):
    ...
    error (str|None).
    """
    mono_files = _list_monolithic_files(base_dir)
...
def create_monolithic_document(
    ...
    if dry_run:
        ...
        prior = _list_monolithic_files(base_dir)
        if prior:
